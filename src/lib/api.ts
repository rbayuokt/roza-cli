...
const parseApiResponse = <T extends z.ZodTypeAny>(
  ...
  if (parsedEnvelope.data.code !== 200) {
    throw new Error(`API ${parsedEnvelope.data.code}: ${parsedEnvelope.data.status}`);
  }
...
export const fetchTimingsByCity = async (opts: FetchByCityOptions): Promise<PrayerData> => {
...
export const fetchNextPrayer = async (opts: FetchNextPrayerOptions): Promise<NextPrayerData> => {
  ...
  return fetchAndParse(`${API_BASE}/nextPrayer/${date}?${params}`, NextPrayerDataSchema);
};
...
export const fetchMethods = async (): Promise<MethodsResponse> => {
  ...
  const cleaned: Record<string, CalculationMethod> = {};
  for (const [key, value] of Object.entries(raw)) {
    const parsed = CalculationMethodSchema.safeParse(value);
    if (parsed.success) {
  ...
  if (Object.keys(cleaned).length === 0) {
    throw new Error('No valid calculation methods returned by API.');
  }
